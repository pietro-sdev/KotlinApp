...
import { SelectCorrectURL, SelectUserType, SECRETARIA } from "../../utils";
...
export default function NavBarSecretaria() {
  ...
  const getUserInformation = (token, storageUserId) => {
    api
      ...
      .then((response) => {
        ...
        if (SelectUserType(response.data) !== SECRETARIA) {
  ...
  useEffect(() => {
    setComponent(
      ...
      >
        ...
        />
        <StyledLink
          onClick={() => navigate("/secretaria/crudpacientes")}
          underline={"hover"}
          text={"Pacientes"}
          themeDefault={false}
        />
  ...
  return (
    <Stack direction="column" spacing={5} sx={{ minWidth: "800px" }}>
      <Stack
        direction="row"
