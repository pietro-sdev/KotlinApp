...
import { PROFISSIONAL, SelectCorrectURL, SelectUserType } from "../../utils";
  ...
  const getUserInformation = (token, storageUserId) => {
    api
      ...
      .then((response) => {
        ...
        if (SelectUserType(response.data) !== PROFISSIONAL) {
  ...
  useEffect(() => {
    setComponent(
      ...
      >
        ...
        />
        <StyledLink
          onClick={() => navigate("/profissional/crudpacientes")}
          underline={"hover"}
          text={"Pacientes"}
          themeDefault={false}
        />
        <StyledLink
          onClick={() => navigate("/profissional/crudconsultamedico")}
          underline={"hover"}
          text={"Consultas"}
          themeDefault={false}
        />
